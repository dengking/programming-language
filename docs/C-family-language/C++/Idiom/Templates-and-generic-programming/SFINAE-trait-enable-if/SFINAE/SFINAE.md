...
简而言之，template是只有在使用的情况下才会被“instantiation”，SFINAE控制了对template的选择，进而实现conditional compiling（conditional compiling又可以称为：compile-time dispatch）; SFINAE 超过type的限制，允许programmer灵活地进行dispatch，比如：
