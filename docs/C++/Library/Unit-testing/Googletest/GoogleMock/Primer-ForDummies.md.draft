(**Note:** If you get compiler errors that you don't understand, be sure to consult [Google Mock Doctor](https://github.com/google/googletest/blob/master/googlemock/docs/FrequentlyAskedQuestions.md#how-am-i-supposed-to-make-sense-of-these-horrible-template-errors).)

# What Is Google C++ Mocking Framework?

When you write a prototype or test, often it's not feasible(可行的) or wise to rely on real objects entirely. A **mock object** implements the same interface as a **real object** (so it can be used as one), but lets you specify at run time how it will be used and what it should do (which methods will be called? in which order? how many times? with what arguments? what will they return? etc).
...
- You are stuck with a sub-optimal design and wish you had done more prototyping before it was too late, but prototyping in C++ is by no means "rapid"(但C ++中的原型设计绝不是“快速”).
- Your tests are slow as they depend on too many libraries or use expensive resources (e.g. a database).
- Your tests are brittle(脆弱的) as some resources they use are unreliable (e.g. the network).
...
- a *testing* tool to cut your tests' outbound dependencies and probe(探测) the interaction between your module and its collaborators.
...
2. Take a *virtual* function of `Turtle` (while it's possible to [mock non-virtual methods using templates](https://github.com/google/googletest/blob/master/googlemock/docs/CookBook.md#mocking-nonvirtual-methods), it's much more involved(复杂的)). Count how many arguments it has.
...
If you want to use something other than Google Test (e.g. [CppUnit](http://sourceforge.net/projects/cppunit/) or [CxxTest](https://cxxtest.com/)) as your testing framework, just change the `main()` function in the previous section to:
...
A better solution is to use Google Test's [event listener API](https://github.com/google/googletest/blob/master/googletest/docs/advanced.md#extending-googletest-by-handling-test-events) to report a test failure to your testing framework properly. You'll need to implement the `OnTestPartResult()` method of the event listener interface, but it should be straightforward.
...
A list of built-in matchers can be found in the [CheatSheet](https://github.com/google/googletest/blob/master/googlemock/docs/CheatSheet.md). For example, here's the `Ge` (greater than or equal) matcher:
...
We've seen `AtLeast(n)` as an example of fuzzy cardinalities earlier. For the list of built-in cardinalities you can use, see the [CheatSheet](https://github.com/google/googletest/blob/master/googlemock/docs/CheatSheet.md).
...
What can we do inside `WillOnce()` besides `Return()`? You can return a reference using `ReturnRef(variable)`, or invoke a pre-defined function, among [others](https://github.com/google/googletest/blob/master/googlemock/docs/CheatSheet.md#actions).
...
Instead of returning 100, 101, 102, ..., consecutively, this mock function will always return 100 as `n++` is only evaluated once. Similarly, `Return(new Foo)` will create a new `Foo` object when the `EXPECT_CALL()` is executed, and will return the same pointer every time. If you want the side effect to happen every time, you need to define a custom action, which we'll teach in the [CookBook](https://github.com/google/googletest/blob/master/googlemock/docs/CookBook.md).
...
(What if you care about the relative order of some of the calls, but not all of them? Can you specify an arbitrary partial order? The answer is ... yes! If you are impatient, the details can be found in the [CookBook](https://github.com/google/googletest/blob/master/googlemock/docs/CookBook.md#expecting-partially-ordered-calls).)
...
Then, if you feel like increasing your mock quotient, you should move on to the [CookBook](https://github.com/google/googletest/blob/master/googlemock/docs/CookBook.md). You can learn many advanced features of Google Mock there -- and advance your level of enjoyment and testing bliss.