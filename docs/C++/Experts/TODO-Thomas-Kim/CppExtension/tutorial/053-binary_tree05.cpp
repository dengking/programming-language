...
class binary_node
{
    ...
    public:
        ...
        bool graft(node_ptr_t node_ptr) // node_ptr_t == std::unique_ptr<binary_node>
        {
            if( node_ptr->m_value == this->m_value)
                return false;
            else if(node_ptr->m_value < this->m_value)
            {
                if(this->m_left)
                    // this is recursion
                    return this->m_left->graft( std::move(node_ptr) );
                else
                {
            ...
            else // node_ptr->m_value > this->m_value
            {
                if(this->m_right)
                    // this is also recursion
                    return this->m_right->graft(std::move(node_ptr));
                else
                {
                    node_ptr->m_parent = this;
        ...
        static bool remove_node(node_ptr_t& root_ptr, ElementType value)
        {
            ...
            if(!ptr->m_parent)
            {
                ...
                if(right_child) // new root
                {
                    ...
                    if(left_child)
                        root_ptr->graft(std::move(left_child));
