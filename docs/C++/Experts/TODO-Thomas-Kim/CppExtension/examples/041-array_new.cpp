...
public:
    ...
    explicit dynamic_array_new(size_t size = 1)
        : m_size{size}
    {
    ...
    dynamic_array_new &operator=(const dynamic_array_new &right_hand_side)
    {
        ...
        // is defined for class dynamic_array_new,
...
void do_not_do_this()
{
    ...
    std::vector< dynamic_array_new<int> > jagged_array(count); // don't do this

    stream << "At this point, default constructor of dynamic_array_new is called "
        << count << " times.\n"<< endl;

    for(size_t i = 0; i < jagged_array.size(); ++i)
        jagged_array[i] = dynamic_array_new<int>{ i + 1 };

    stream << "In the for loop, default constructor of dynamic_array_new is called "
...
void is_still_is_not_perfect()
{
    ...
    std::vector< dynamic_array_new<int> > jagged_array;
    jagged_array.reserve(count);

    for(size_t i = 0; i < jagged_array.capacity(); ++i)
        jagged_array.emplace_back(dynamic_array_new<int>{ i + 1 });
}
...
void please_use_this_method()
{
    ...
    for(size_t i = 0; i < jagged_array.capacity(); ++i)
    {
        ...
        // i + 1 is argument for dynamic_array_new.

            // now std::vector creates dynamic_array_new
            // internally, so, no move... only count times of default
            // constructor of dynamic_array_new is called.

        for(size_t j = 0; j < jagged_array.back().size(); ++j)
            jagged_array.back()[j] = (int)j;
    }

    // note that the constructor of dynamic_array_new{size_t}
...
void test_iterator()
{
    dynamic_array_new<int> array{10};
