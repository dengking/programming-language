	...
	var jprp = {
		...
		getEndpointURL: function( URL ) {
			...
			if ( undefined != $( '#jp-relatedposts' ).data( 'exclude' ) ) {
				args += '&relatedposts_exclude=' + encodeURIComponent( $( '#jp-relatedposts' ).data( 'exclude' ) );
			}
		...
		generateMinimalHtml: function( posts ) {
			var self = this;
			var $div = $( '<div class="jp-relatedposts-items jp-relatedposts-items-minimal">' );
			...
			$.each( posts, function( index, post ) {
				...
				$p
					...
					.append(
						$( '<span class="jp-relatedposts-post-title"></span>' )
							.text( post.title )
							.wrapInner( anchor[0] + anchor[1] ),
						$( '<span class="jp-relatedposts-post-context">' ).text( post.context )
					);
		...
		generateVisualHtml: function( posts ) {
			var self = this;
			var $div = $( '<div class="jp-relatedposts-items jp-relatedposts-items-visual">' );
		...
		getTrackedUrl: function( anchor ) {
			...
			args += '&relatedposts_origin=' + encodeURIComponent( $( anchor ).data( 'origin' ) );
			args += '&relatedposts_position=' + encodeURIComponent( $( anchor ).data( 'position' ) );
